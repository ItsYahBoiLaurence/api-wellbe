...
export class CheckInService {
  ...
  async getUserCheckInStatus(user_details: JwtPayload) {
    ...
    if (latest_batch.is_completed === true)
      return {
        is_batch_available: false,
        check_in_count: null,
        has_pending_questions: null,
    ...
    const hasPendingQuestion = this.hasMissedQuestions(
      employee_progress.set_participation as [],
      latest_batch.current_set_number,
    );
    ...
    const nextAvailableQuickCheck =
      latest_batch.current_set_number === 5
        ? null
  ...
  async getUserStatus(user_details: JwtPayload) {
    ...
    if (!employee_progress) {
      throw new ConflictException('User is not included to the batch!');
    }
    ...
    const startDate = new Date(latest_batch.start_date);
    const frequency = latest_batch.frequency;
    const daysToAdd = frequency == 'DAILY' ? 1 : frequency == 'WEEKLY' ? 7 : 0;

    if (Array.isArray(employee_progress.set_participation)) {
      const participation = employee_progress.set_participation as boolean[];
      ...
      participation.forEach((set, index) => {
        ...
        check_ins.push({
          id: index + 1,
          check_in_date: date.toLocaleDateString('en-US', {
            month: 'long',
            day: 'numeric',
            timeZone: 'UTC',
          }),
